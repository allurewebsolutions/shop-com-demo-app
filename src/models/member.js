...
export default {
  ...
  reducers: {
    ...
    setUserDetails(state, payload) {
      const {
        firstName, lastName, profilePhoto, signedUp, role,
      } = payload;
  ...
  effects: dispatch => ({
    ...
    signUp(formData) {
      const {
        email, password, password2, firstName, lastName, profilePhoto,
      } = formData;
      ...
      return new Promise(async (resolve, reject) => {
        ...
        if (!profilePhoto) return reject({ message: errorMessages.missingProfilePhoto });
